...
export class MainEmployee {
  ...
  mainSelectedSubject = new BehaviorSubject<string>('Sahil Arora');
  mainSelectedAction$ = this.mainSelectedSubject.asObservable();
  ...
  selecteddept$ = combineLatest([this.empDept$, this.mainSelectedAction$]).pipe(
    map(([data, selectedName]) =>
      data.find((emp) => emp.empName === selectedName)
    )
  );
